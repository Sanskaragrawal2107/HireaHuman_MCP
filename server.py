...
async def _record_profile_views_batch(profile_ids: list[str], source: str = "chatbot_search") -> None:
    """Record multiple profile views in a single bulk API call."""
    if not profile_ids:
        return
    url = f"{API_BASE}/profile_views"
    records = [{"profile_id": pid, "source": source} for pid in profile_ids]
    try:
        async with httpx.AsyncClient(timeout=API_TIMEOUT) as client:
            response = await client.post(url, headers=HEADERS, json=records)
            response.raise_for_status()
            logger.info(f"Recorded {len(profile_ids)} profile views (source={source})")
    except Exception as e:
        logger.warning(f"Failed to record batch profile views: {e}")
...
def _format_candidate(p: dict, brief: bool = False, for_hiring: bool = False) -> dict:
    ...
    email_val = contact_email.strip() if (contact_email and isinstance(contact_email, str) and contact_email.strip()) else None
    ...
    base = {
        ...
        "email": email_val or f"{p.get('handle')}@hireahuman.ai",
...
async def search_candidates(
    ...
    try:
        ...
        # Record profile views for all returned candidates
        view_ids = [c.get("id") for c in all_limited if c.get("id")]
        if view_ids:
            await _record_profile_views_batch(view_ids, source="chatbot_search")
...
async def list_available_candidates(
    ...
    try:
        ...
        # Record profile views for listed candidates
        view_ids = [c.get("id") for c in candidates if c.get("id")]
        if view_ids:
            await _record_profile_views_batch(view_ids, source="chatbot_browse")
...
async def search_by_skills(
    ...
    try:
        ...
        # Record profile views for matched candidates
        view_ids = [c.get("id") for c in result if c.get("id")]
        if view_ids:
            await _record_profile_views_batch(view_ids, source="chatbot_skill_search")
...
async def check_candidate_availability(handle: str) -> str:
    ...
    try:
        params = {
            ...
            "select": "handle,display_name,employment_status,location,preferred_location,linkedin_url,github_url,bluetech_badge,role_title,contact_email",
            "limit": "1",
        }
        profiles = await _query_profiles(params)
